...
function toRealtimeUserEvent(data: unknown): RealtimeUserEvent {
...
export function UserDashboard() {
  ...
  useEffect(() => {
    const handleEvent = (data: unknown) => {
      const event = toRealtimeUserEvent(data);
      toast({
        message: `Real-time: ${event.action || "update"} — ${event.name || "User"}`,
        type: "info",
        duration: 3000,
      });
      refetch();
          ...
          <p className="text-muted-foreground mt-1">
            ...
            {authUser && (
              <span className="ml-2 text-xs">
                · Logged in as <strong>{authUser.name}</strong> ({authUser.role}
                )
              </span>
            )}
          </p>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {users.find((u) => u.id === deleteConfirmId)?.name}"? This action
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
